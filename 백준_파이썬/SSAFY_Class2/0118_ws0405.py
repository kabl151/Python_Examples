...
def is_validation(j):
    deny_count = 0
    if (user_data[j] in blood_types):
        deny_count += 1
        
    elif (user_data[j]['company'] in black_list):
        deny_count += 1
        
    elif ('@' in user_data[j]['mail']):
        deny_count += 1
        
    elif (len(user_data[j]['name'])>=2 and len(user_data[j]['name'])<=30):
        deny_count += 1
        
    elif (len(user_data[j]['website'])>=1):
        deny_count += 1
        
        
    if deny_count == 1:
        return False
    #디나이 항목별 인자들 만들어서 카운트 시켜버리기.